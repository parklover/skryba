    ...
    performFormulaOperation(args) {
        ...
        switch (action) {
            ...
            case 'renameUpdation':
                ...
                break;
            case 'addSheet':
                this.sheetInfo.push({ visibleName: args.sheetName, sheet: args.sheetName, index: args.index });
                break;
            case 'getSheetInfo':
...
class WorkbookSort {
    ...
    initiateSortHandler(eventArgs) {
        ...
        getData(this.parent, `${sheet.name}!${address}`, true).then((jsonData) => {
            ...
            dataManager.executeQuery(query).then((e) => {
                ...
                let eventArgs = { range: address, sortOptions: args.sortOptions };
    ...
    sortComparer(sortDescriptor, caseSensitive, x, y) {
        ...
        return comparer(x ? x.value : x, y ? y.value : y, this.parent.locale, caseOptions);
    }
    /**
...
class WorkbookCellFormat {
    ...
    format(args) {
        ...
        let triggerEvent = typeof (args.range) !== 'object' && args.onActionUpdate;
...
function isHiddenRow(sheet, index) {
    return sheet.rows[index] && (sheet.rows[index].hidden || (sheet.rows[index].height !== undefined && sheet.rows[index].height === 0));
}
/**
 ...
 */
function getData(context, address, columnWiseData) {
...
function setResize(index, value, isCol, parent) {
    ...
    if (isCol) {
        ...
        let hdrFntSize = this.findMaxValue(parent.getRowHeaderTable(), hdrClone, false, parent) + 1;
        ...
        let cntFntSize = this.findMaxValue(parent.getContentTable(), contentClone, false, parent) + 1;
...
class Clipboard {
    ...
    paste(args) {
        ...
        let cSID = this.copiedInfo && this.copiedInfo.sId ? this.copiedInfo.sId : this.parent.activeSheetTab;
        let eventArgs = {
            reqType: 'paste',
...
class Resize {
    ...
    wireEvents(args) {
        let rowHeader = isNullOrUndefined(this.parent.getRowHeaderContent()) ?
            ...
            if ((trgt.offsetHeight < 10 && e.offsetY < Math.ceil((trgt.offsetHeight - 2) / 2)) || (e.offsetY < 5 &&
                ...
                if (trgt.parentElement.previousElementSibling || isHiddenRow(sheet, prevIdx)) {
                    if (e.type === 'dblclick' && isHiddenRow(sheet, prevIdx)) {
                        let selectRange = getRangeIndexes(this.parent.getActiveSheet().selectedRange);
                        let eventArgs;
    ...
    setRowHeight(rowIdx, viewportIdx, height) {
        let sheet = this.parent.getActiveSheet();
    ...
    resizeStart(index, value, isCol, isFit) {
        setResize(index, value, isCol, this.parent);
        let action = isFit ? 'resizeToFit' : 'resize';
        let eventArgs = isCol ? { index: index, width: value, isCol: isCol } : { index: index, height: value, isCol: isCol };
        ...
        switch (options.action) {
            case 'sorting':
                ...
                sortArgs.promise.then((args) => {
                    this.parent.serviceLocator.getService('cell').refreshRange(getIndexesFromAddress(args.range));
                });
                break;
            case 'cellSave':
...
class ShowHide {
    ...
    showHideRow(args) {
        ...
        if (args.hide) {
            ...
            let hRow;
            let rowRenderer = this.parent.serviceLocator.getService('row');
            let rTBody;
            let tBody = content.tBodies[0];
            if (sheet.showHeaders) {
                rTBody = rowHdr.tBodies[0];
            }
            let endRow = args.startRow;
            ...
            getUpdateUsingRaf(() => {
                if (sheet.showHeaders) {
                    if (idx !== 0 && !isHiddenRow(this.parent.getActiveSheet(), endRow - 1)) {
...
class SheetTabs {
    ...
    createSheetTabs() {
        ...
        for (let i = 0; i < sheetCount; i++) {
            ...
            let arg = { action: 'addSheet', sheetName: sheetName, index: i + 1 };
    ...
    addSheetTab(args) {
        ...
        this.parent.element.focus();
        let arg = { action: 'addSheet', sheetName: sheetName, index: this.parent.getActiveSheet().id };
        this.parent.notify(workbookFormulaOperation, arg);
    }
    updateSheetTab(args) {
...
class Open {
    ...
    openSuccess(response) {
        ...
        this.parent.serviceLocator.getService(beginCompleteEvents).completeAction({ response: response }, 'import');
        this.parent.renderModule.refreshSheet();
...
class ContextMenu$1 {
    ...
    setHideShowItems(items, l10n, layout) {
            ...
            buttons: [{
                    ...
                    click: () => {
                        ...
                        if (!this.validateError(data, element, errorElem)) {
                            ...
                            this.parent.sort({ sortDescriptors: data, containsHeader: hasHeader, caseSensitive: caseCheckbox.checked });
                        }
                    }
                }]
        });
...
class BasicModule {
     ...
     */
    constructor() {
        Spreadsheet.Inject(Ribbon$$1, FormulaBar, SheetTabs, Selection, Edit, KeyboardNavigation, KeyboardShortcut, Clipboard, DataBind, Open, ContextMenu$1, Save, NumberFormat, CellFormat, Formula, Sort, Resize, CollaborativeEditing);
    }
    /**
     * For internal use only - Get the module name.
     * @private
...
class AllModule {
     ...
     */
    constructor() {
        Spreadsheet.Inject(Ribbon$$1, FormulaBar, SheetTabs, Selection, Edit, KeyboardNavigation, KeyboardShortcut, Clipboard, DataBind, Open, Save, NumberFormat, CellFormat, Formula, Sort, Resize, CollaborativeEditing);
    }
    /**
     * For internal use only - Get the module name.
     * @private
...
const dialog = 'dialog';
const beginCompleteEvents = 'beginCompleteEvents';
const fontColor = {
    'Custom': [
...
class BeginCompleteEvents {
    ...
    initializeActionBegin() {
        ...
        this.parent.beforeCellFormat = (args) => {
            if (cellFormat) {
                cellFormat.apply(this, [args]);
            }
            this.beginAction(args, 'cellFormat');
        };
        let beforeOpen = this.parent.beforeOpen;
        this.parent.beforeOpen = (args) => {
            if (beforeOpen) {
                beforeOpen.apply(this, [args]);
            }
            this.beginAction(args, 'beforeOpen');
        };
        let beforeSave = this.parent.beforeSave;
        this.parent.beforeSave = (args) => {
            if (beforeSave) {
                beforeSave.apply(this, [args]);
            }
            this.beginAction(args, 'beforeSave');
        };
        let beforeSort = this.parent.beforeSort;
        this.parent.beforeSort = (args) => {
            if (beforeSort) {
                beforeSort.apply(this, [args]);
            }
            this.beginAction(args, 'beforeSort');
        };
    }
    initializeActionComplete() {
        ...
        this.parent.sortComplete = (args) => {
            if (sortComplete) {
                sortComplete.apply(this, [args]);
            }
            this.completeAction(args, 'sorting');
        };
        ...
        this.parent.cellSave = (args) => {
            ...
            if (args.oldValue !== args.value) {
                this.completeAction(args, 'cellSave');
            }
        };
    }
    beginAction(args, action) {
        this.parent.trigger('actionBegin', { action: action, args: args });
    }
    completeAction(args, action) {
